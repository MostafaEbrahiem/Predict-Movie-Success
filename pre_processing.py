...
def process (data,ch):
    ...
    #X = data
    #c_t = ColumnTransformer(transformers=[('encoder', OneHotEncoder(), [11])], remainder='passthrough')
    ...
    del data['Genres']
    if ch==1:
        #dump(data, open('c_data.pkl', 'wb'))
        dump(data, open('c_t_data.pkl', 'wb'))
    else:
        #dump(data, open('data.pkl', 'wb'))
        dump(data, open('T_data.pkl', 'wb'))
